...
구간에 나눠 준 뒤 그 예산 안에서 문장을 쓴다. **속도는 짐작하지 말고 실측값을 쓴다** —
`edge` 한국어 `rate +6%` 기준 **초당 약 6.3자**(5분 ≈ 1,800자). 다른 목소리·속도를 쓰면
`--only tts` 로 한 구간만 뽑아 `음성 길이 ÷ 글자수` 로 그 조합의 값을 구한다.
어림으로 "초당 5자"를 잡았다가 4분 27초가 나온 적이 있다.
...
0.8초 이상 무음이 잡히면 리듬이 늘어진 것이다. 이때 클립 `duration`을 줄이는 것은 답이 아니다 —
TTS 제공자가 mp3 앞뒤에 붙이는 무음은 합성 직후 자동으로 잘리므로(`lib/tts.js`), 무음이
잡혔다면 대개 `duration`을 오디오보다 길게 고정해 둔 경우다. `duration`을 빼서 하한
(오디오+0.5초)에 맡긴다.
